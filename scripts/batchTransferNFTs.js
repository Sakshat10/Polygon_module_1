...
async function main() {
  try {
    const ethereumContractAddress = process.env.CONTRACT_ADDRESS; // Ethereum UNFTC contract address
    ...
    const ethereumContract = await ethers.getContractAt(
      "Sakshat",
    ...
    await ethereumContract.setApprovalForAll(fxERC721RootTunnel.address, true);
